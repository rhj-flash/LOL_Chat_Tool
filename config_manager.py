...
class ConfigManager:
    """
    负责管理两个配置文件的类：
    1. messages.json：用于存储消息组和备注。
    2. config.json：用于存储通用程序配置，如热键。
    """

    def __init__(self, messages_file="messages.json", config_file="config.json"):
        """
        初始化 ConfigManager，设置配置文件路径并加载消息和通用配置。
        :param messages_file: 消息配置文件名。
        :param config_file: 通用配置文件名。
        """
    ...
    def load_config(self) -> dict:
        """
        从 config.json 文件加载通用配置。若文件不存在或加载失败，则返回一个空字典。
        :return: 配置字典。
        """
        if not os.path.exists(self.config_file):
            logging.info("通用配置文件不存在，创建新的空配置文件。")
        ...
        try:
            ...
            logging.error(f"加载通用配置失败: {str(e)}。将返回空字典。")
            return {}
    ...
    def add_message(self, new_note: str, new_message_str: str):
        """
        添加新消息到消息列表。
        ...
        """
        if new_message_str is None or not new_message_str.strip():
        ...
        # 这里为了防止误判，只检查备注和第一行是否相同
        for msg in self.messages:
            if msg.get('note') == new_note and msg.get('lines', [])[0] == new_message_lines[0]:
    ...
    def edit_message(self, index: int, new_note: str, new_message_str: str):
        """
        更新指定索引处的消息。
